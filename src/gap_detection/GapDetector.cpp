            ...
            if (mark_to_start && observed_gaps.at(i).isAxial() && observed_gaps.at(i).isLeftType())
            {
                ...
                if (!mark_to_start)
                {
                    if (observed_gaps.at(i).isAxial())
                    {
                        ...
                        {
                            float curr_left_flipped_dist = observed_gaps[i].RDist();
                            int erase_counter = 0;
                            int last_mergable = -1;
                            ...
                            if (last_mergable != -1) {
                                ...
                                second_gap.back().addRightInformation(observed_gaps[i].LFlippedIdx(), observed_gaps[i].RDist());
                            } else {
                    ...
                    {
                        // If not axial gap, 
                        float curr_left_flipped_dist = observed_gaps.at(i).RDist();
                        if (std::abs(curr_left_flipped_dist - second_gap.back().RFlippedDist()) < 0.2 && second_gap.back().isAxial() && second_gap.back().isLeftType())
                        {
                            second_gap.back().addRightInformation(observed_gaps[i].LFlippedIdx(), observed_gaps[i].RDist());
                        } else {
                            second_gap.push_back(observed_gaps[i]);
